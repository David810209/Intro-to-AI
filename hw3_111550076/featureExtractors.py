...
class SimpleExtractor(FeatureExtractor):
    ...
    def getFeatures(self, state, action):
        ...
        # If there are active ghosts nearby, set the "escape" feature to 1.0
        # indicating the agent should try to move away
        if features["#-of-active-ghosts-1-step-away"]:
            features["escape"] = 1.0
        else:
            # If no active ghosts are nearby, check for other conditions
            # If a capsule is present and there are no scared ghosts, set the "eats-capsule" feature to 1.0
            if (next_x, next_y) in capsules and not scared:
                features["eats-capsule"] = 1.0
            # If there is food at the next position, set the "eats-food" feature to 1.0
